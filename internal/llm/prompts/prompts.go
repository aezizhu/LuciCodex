...
func GenerateSurvivalPrompt(maxCommands int) string {
	...
	b.WriteString("- If the user input is conversational (e.g. 'hi', 'hello', 'thanks') or asks a general question, return an empty 'commands' list and a helpful 'summary'.\n")
	b.WriteString("- Do NOT suggest commands unless the user explicitly asks for an action or status check.\n")
	b.WriteString("- Common commands (ONLY if requested):\n")
